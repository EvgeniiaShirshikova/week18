...
function createCards() {
  ...
  const superheroesContainer = document.querySelector(".container");
  // указываем шаблон, по которому нужно создать карточку
  const superheroTemplate = document.querySelector("#superhero-template").content;
  // функция для заполнения карточки
  superheroes.forEach(function renderCard({ name, universe, alterego, occupation, friends, superpowers, url, info }) {
    // объявляем карточку, присваиваем ей шаблон и прописываем, что будем ее клонировать
    const superheroElement = superheroTemplate.querySelector(".superhero-card").cloneNode(true);
    // указываем в какую часть карточки подтягивать какую информацию из массива
    superheroElement.querySelector(".superhero-card__name").textContent = name;
    superheroElement.querySelector(".superhero-card__universe").textContent = `Вселенная: ${universe}`;    
    superheroElement.querySelector(".superhero-card__alterego").textContent = `Альтер эго: ${alterego}`;
    superheroElement.querySelector(".superhero-card__occupation").textContent = `Род деятельности: ${occupation}`;
    superheroElement.querySelector(".superhero-card__friends").textContent = `Друзья: ${friends}`;
    superheroElement.querySelector(".superhero-card__superpowers").textContent = `Суперсилы: ${superpowers}`;
    superheroElement.querySelector(".superhero-card__img").src = url;
    superheroElement.querySelector(".superhero-card__info").textContent = `Описание: ${info}`;
    // добавляем карточки
    superheroesContainer.append(superheroElement);
  });
}
createCards();
