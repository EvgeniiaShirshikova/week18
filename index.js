...
function renderCard({ name, universe, alterego, occupation, friends, superpowers, url, info, id, rating }) {
  ...
  const template = 
  `
  <h2 class="superhero-card__name">${name}</h2>
        <div class="superhero-card__universe">Вселенная: ${universe}</div>
        <div class="superhero-card__alterego">Альтер эго: ${alterego}</div>        
        <div class="superhero-card__occupation">Род деятельности: ${occupation}</div>
        <div class="superhero-card__friends">Друзья: ${friends}</div>
        <div class="superhero-card__superpowers">Суперсилы: ${superpowers}</div>
        <img class="superhero-card__img" src="${url}" alt="${id}">
